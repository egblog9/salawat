...
export const AudioReminderCard: React.FC<AudioReminderCardProps> = ({
  ...
  const handleToggle = () => {
    if (!enabled) {
      // Activating: ensure user interaction registers for audio autoplay policy
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[11px] sm:text-xs font-bold px-2.5 py-0.5 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/40 shadow-sm flex items-center gap-1">
                <span>✨ تجريبي</span>
              </span>
              {enabled && (
            ...
            <p className="text-xs text-stone-400 mt-0.5">
              تذكير صوتي دوري ومريح بالصلاة على النبي ﷺ أثناء بقاء الموقع مفتوحاً
            </p>
          </div>
        </div>
      ...
      {enabled ? (
          ...
          <div className="bg-stone-950/70 rounded-2xl p-4 border border-stone-800/80 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{isTestingSound ? "جاري الاستماع للتسجيل..." : "اختبار الصوت"}</span>
              </button>
            </div>

          </div>

          {/* Tranquil Recitation Details */}
          <div className="flex items-center gap-2 text-[11px] text-stone-400 pt-1">
            <Sparkles className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
            <span>
              نص التسجيل: <strong className="text-amber-200 font-amiri font-normal text-xs">«اللهم صلِّ وسلم وبارك على نبينا محمد ﷺ»</strong> (صوت عربي فصيح، هادئ ومريح).
            </span>
          </div>
        ...
        /* Hint when disabled */
        <div className="mt-4 pt-4 border-t border-stone-800/60 flex items-center justify-between gap-3 text-xs text-stone-400">
          <div className="flex items-center gap-2">
            <Info className="w-4 h-4 text-emerald-500/70 flex-shrink-0" />
            <span>فعّل التذكير الصوتي لتذكيرك بالصلاة على الحبيب ﷺ بانتظام كل 10 دقائق أو بالمدة التي تختارها.</span>
          </div>
          <button
            onClick={onTestSound}
            className="text-[11px] text-amber-400/90 hover:text-amber-300 hover:underline flex-shrink-0 cursor-pointer"
          >
            تجربة التسجيل الصوتي
          </button>
        </div>
      )}
